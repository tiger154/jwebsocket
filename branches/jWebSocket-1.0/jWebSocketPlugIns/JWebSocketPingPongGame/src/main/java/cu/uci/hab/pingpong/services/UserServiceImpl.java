 ...
 */
package cu.uci.hab.pingpong.services;

import com.mongodb.BasicDBObject;
import com.mongodb.DBCollection;
import com.mongodb.DBCursor;
import com.mongodb.DBObject;
import cu.uci.hab.pingpong.api.IUserService;
...
public class UserServiceImpl extends TimerTask implements IUserService {
    ...
    public DBCursor getProfileList() {
        DBCursor lProfile = mCollection.find(new BasicDBObject(), new BasicDBObject("user", true).append("wins", true).append("lost", true).append("_id", false));
        return lProfile;
    }

    @Override
    public boolean isPwdCorrect(String aUserName, String aPwd) {
        DBObject lUser = mCollection.findOne(new BasicDBObject().append("user", aUserName).append("pwd", aPwd));
        if (lUser == null) {
            return false;
        }
    ...
    public void updateValue(String aUserName, int aWins, int aLost) {
        ...
        if (aWins == 1) {
            lData.put("wins", aWins + Integer.parseInt(lDBObject.get("wins").toString()));
            lData.put("lost", aLost + Integer.parseInt(lDBObject.get("lost").toString()));
        } else {
            lData.put("wins", aWins + Integer.parseInt(lDBObject.get("wins").toString()));
            lData.put("lost", aLost + Integer.parseInt(lDBObject.get("lost").toString()));
        }
    ...
    public void removeUser(int aDay) {
        ...
        DBCursor lProfile = mCollection.find(new BasicDBObject(), new BasicDBObject("date", true).append("_id", false));
        for (DBObject lDB : lProfile) {
            Date lLastDate = (Date) lDB.get("date");
            Date lCurrentDate = new Date();
            long lDifference = (lCurrentDate.getTime() - lLastDate.getTime()) / lMllsecsPerDay;
            if (lDifference >= aDay) {
