...
import org.jwebsocket.gaming.Match;
...
public class PingpongMatch extends Match implements Runnable {
		...
		try {
			while (true) {
				if (mCont >= 0) {
					...
					int lLeftPlayer = (int) (mPingpongPlayerList.get(0).getPosition().getX() + mPingpongPlayerList.get(0).getDimension().getWidth());
					int lRightPlayer = (int) (mPingpongPlayerList.get(1).getPosition().getX() - mBall.getDimension().getWidth());
					int lBottomBall = mBall.getPosition().getY() + mBall.getDimension().getHeight();
					mSpeed = 50;
					if (mBall.getPosition().getX() <= mPingpongStage.getStageBorderLeft()) {
						mPingpongPlayerList.get(1).setPlayerScore(mPingpongPlayerList.get(1).getPlayerScore() + 1);
						mBall.notify(new Left(this), null, true);
						mActServ = -1;
						initBall();
						if (mPingpongPlayerList.get(1).getPlayerScore() >= mScore) {
							mBall.notify(new GameOver(this, true), null, true);
							pause();
						}
						mSpeed = 1000;
					} else if (mBall.getPosition().getX() + mBall.getDimension().getWidth() + 12 >= mPingpongStage.getStageBorderRaight()) {
						mPingpongPlayerList.get(0).setPlayerScore(mPingpongPlayerList.get(0).getPlayerScore() + 1);
						mBall.notify(new Right(this), null, true);
						mActServ = 1;
						initBall();
						if (mPingpongPlayerList.get(0).getPlayerScore() >= mScore) {
							mBall.notify(new GameOver(this, true), null, true);
							pause();
						}
						mSpeed = 1000;
					}
					if (mBall.getPosition().getY() <= mPingpongStage.getStageBorderTop() + 3 || mBall.getPosition().getY() >= mPingpongStage.getStageBorderBottom() - 20) {
						mBall.setMoveBallY(mBall.getMoveBallY() * -1);
						mBall.notify(new Edge(this), null, true);
						mBall.notify(new Sound(this, 0), null, true);
					} else if (mBall.getPosition().getX() - 2 <= lLeftPlayer && lBottomBall >= mPingpongPlayerList.get(0).getPosition().getY() && mBall.getPosition().getY() <= mPingpongPlayerList.get(0).getPosition().getY() + mPingpongPlayerList.get(0).getDimension().getHeight()) {
						mBall.setMoveBallX(mBall.getSpeedBall());
						hitBall(mPingpongPlayerList.get(0));
						mBall.notify(new PlayerLeft(this), null, true);
						mBall.notify(new Sound(this, 0), null, true);
					} else if (mBall.getPosition().getX() >= lRightPlayer-2 && lBottomBall >= mPingpongPlayerList.get(1).getPosition().getY() && mBall.getPosition().getY() <= mPingpongPlayerList.get(1).getPosition().getY() + mPingpongPlayerList.get(1).getDimension().getHeight()) {
						mBall.setMoveBallX(-mBall.getSpeedBall());
					...
					mBall.getPosition().setX(mBall.getPosition().getX() + mBall.getMoveBallX());
					mBall.getPosition().setY(mBall.getPosition().getY() + mBall.getMoveBallY());
	...
	public void hitBall(PingpongPlayer aPingpongPlayer) {
		...
		int lCenterBar = Math.round(aPingpongPlayer.getPosition().getY() + aPingpongPlayer.getDimension().getHeight() / 2);

		if (lBottomBall + mBall.getSpeedBall() < mPingpongStage.getStageBorderBottom() && lTopBall - mBall.getSpeedBall() > mPingpongStage.getStageBorderTop()) {
			if (lCenterBall < lCenterBar - aPingpongPlayer.getDimension().getHeight() / 2) {
				mBall.setMoveBallY(Math.round(-mBall.getSpeedBall() * 1.5f));
			} else if (lCenterBall > lCenterBar + aPingpongPlayer.getDimension().getHeight() / 2) {
				mBall.setMoveBallY(Math.round(mBall.getSpeedBall() * 1.5f));
			} else if (lCenterBall < lCenterBar - aPingpongPlayer.getDimension().getHeight() / 4) {
				mBall.setMoveBallY(-mBall.getSpeedBall());
			} else if (lCenterBall > lCenterBar + aPingpongPlayer.getDimension().getHeight() / 4) {
				mBall.setMoveBallY(mBall.getSpeedBall());
			} else if (lCenterBall < lCenterBar - aPingpongPlayer.getDimension().getHeight() / 6) {
	...
	public void movePlayer(int aEvent, String aUsername,String aValue) {
		try {
			...
			lPingpongPlayer.notify(new MovePlayer(this, lPingpongPlayer.getPlayerJoystick()), null, true);
		} catch (Exception ex) {
			Logger.getLogger(PingpongMatch.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
