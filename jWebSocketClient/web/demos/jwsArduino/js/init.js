...
function sendCommand(aCmd){
    ...
    if(gRcPlugin != null){
        ...
        showMessage("Arduino Remote Control can not connect to server","warning");
    }
}
...
function startArduinoRemoteControl(){
    ...
    gRcPlugin.startrc({
        OnSuccess: function(aResponse){
            if(aResponse.message != null)
                showMessage(aResponse.message + "<br><br>"+"Disconnect and reconnect the microcontroller, set the port and restart the jWebSocket server.","alert");
        }					
    });    
}
