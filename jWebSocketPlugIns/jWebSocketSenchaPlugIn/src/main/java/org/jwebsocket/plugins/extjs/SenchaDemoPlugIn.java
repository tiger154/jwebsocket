 ...
 *
 * @author Osvaldo Aguilar Lauzurique, (oaguilar, La Habana), Alexander Rojas
 * Hernandez (arojas, Pinar del Rio), Victor Antonio Barzana Crespo (vbarzana,
 * Münster Westfalen)
 *
 */
public class SenchaDemoPlugIn extends TokenPlugIn {
	...
	private void proccessRead(
		...
		try {
			...
			if (lId != null) {
				...
				for (Iterator<User> lItUser = lUsersList.iterator();
						lItUser.hasNext();) {
					User lUser = lItUser.next();
					Token lAuxToken = TokenFactory.createToken();
					lUser.writeToToken(lAuxToken);
					lResultList.add(lAuxToken);
				}
	...
	private void notifyAllConectors(String aNotificationType, String aMessage) {
		...
		for (Iterator<WebSocketConnector> lConnectorIt = mClients.iterator();
				lConnectorIt.hasNext();) {
			WebSocketConnector lConnector = lConnectorIt.next();
