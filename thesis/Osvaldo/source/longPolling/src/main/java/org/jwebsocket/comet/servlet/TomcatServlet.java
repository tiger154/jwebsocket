 ...
 */
public class TomcatServlet extends HttpServlet implements CometProcessor {
    ...
    public TomcatServlet() {
        mLog = Logging.getLogger(TomcatServlet.class);
    }
    ...
    public void init() throws ServletException {
        ...
        mEngine = (CometEngine) JWebSocketFactory.getEngine();
    ...
    public void event(CometEvent aEvent) throws IOException, ServletException {
        if (aEvent.getEventType() == EventType.BEGIN) {
            ...
            try {
                lJson = new JSONObject(lText);
            } catch (Exception ex) {
                mLog.error("Unexpected error parsing input stream to json..." + lText + "..." + " from id" + aEvent.getHttpServletRequest().getSession().getId());
            }
            Map<String, Object> lInputMessage = JSONProcessor.jsonObjectToMap(lJson);
    ...
    /**
     * if the connection message is receive for the firs time,
     * answer with the open ready state else just update de ready state
